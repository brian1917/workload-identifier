...
func main() {
	...
	csvFile := flag.String("input", "umwl_finder_default.csv", "CSV File to be used to identify unmanaged workloads.")
	outputFile := flag.String("output", "umwl_output.csv", "File to write the unmanaged workloads to.")
	incWLs := flag.Bool("w", false, "Include IP addresses already assigned to workloads (typically used in testing only).")
	disableTLS := flag.Bool("x", false, "Disable TLS checking")
	verbose := flag.Bool("v", false, "Verbose output produces a log file and adds an additional column in the output CSV for match reason.")
	dupes := flag.Bool("d", false, "Allow same IP address to have several unmanaged workloda recommendations. Default will use the order in the input CSV.")
	term := flag.Bool("t", false, "Print the CSV to the terminal")
